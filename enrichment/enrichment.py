...
# first (ie library) round and the final round is more indicative of a successful 
# aptamer than just overall abundance in later rounds, since libraries are often not
# truly random, and an initial bias can give a certain sequence a head start over other
# sequences that are actually bind to the target better.  
#
#  Intended to be used on sequence:count files, as generated by the Jellyfish program, 
# or by Biopieces using the "write_tab" function. Sequences are assumed to be unique. 
# A command such as:
#       awk '!(NR%2){print$0"\t"p}{p=$0}' | sed 's/>//g' > [outfile]
# can be used to transform fasta files that use the sequence count as a header
# into the correct input format. If you have tabular date with more fields, pipe
# the input through Cut first to extract the fields containing sequence and count. 
#
